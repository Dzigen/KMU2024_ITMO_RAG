...
class BM25E5Retriever:
    def __init__(self, bm25_candidates=3, e5_candidates=2, docs_bs=4) -> None:
    ...
    def load_e5_model(self, weights_path):
        self.e5_model.load_state_dict(torch.load(weights_path))
    ...
    def search(self, query, tokenized_query=None):
        ...
        e5_docs, scores = self.e5_retrieve(
            tokenized_query, bm25_docs, tokenized_docs)

        return e5_docs, scores
    ...
    #
    def e5_retrieve(self, tokenized_query, bm25_docs, docs_loader):
        all_scores = torch.tensor([], requires_grad=True)
        for doc_batch in docs_loader:
            print("doc_batch - ", doc_batch['input_ids'].shape)

            scores = self.e5_model(tokenized_query['input_ids'], tokenized_query['attention_mask'],
